    ...
    <>
      <div className="px-10 w-[90%]  mx-auto drop-shadow-lg pb-20">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {loadingState==="pending" && Array(4)
            .fill({})
            .map((ingredient, idx) => {
              return (
                <Fragment key={idx}>
                  {/* <p className="break-words">{JSON.stringify(ingredient)}</p> */}
                  <div className="bg-white sm:h-52 rounded shadow-lg flex flex-col sm:flex-row gap-5 select-none border max-w-full sm:max-w-full md:max-w-full w-full lg:max-w-[492px] lg:flex border-red-500">
                    <div className="flex justify-center items-center w-full h-full bg-gray-200 rounded-l sm:w-[200px] dark:bg-gray-400 border-r border-r-yellow-300">
                      <svg
                    ...
                    <div className="flex flex-col flex-1 gap-5 sm:p-2 my-auto justify-center ">
        ...
        {loadingState === "idle" && ingredientsList.length === 0 && (
